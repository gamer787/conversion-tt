...
-- Add receiver_id column to messages
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE conversation_id = target_conversation_id::uuid
  ...
  WHERE m.conversation_id = target_conversation_id::uuid
...
BEGIN
  ...
  SET 
    last_message_at = now(),
    updated_at = now()
  WHERE id = target_conversation_id::uuid;
...
CREATE INDEX IF NOT EXISTS idx_messages_conversation_receiver 
ON messages(conversation_id, receiver_id);
